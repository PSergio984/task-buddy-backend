...
Findings: `research/b01-supabase-connectivity.md`. Key facts: persistent FastAPI + Celery → direct connection (port 5432), session pooler as IPv4 fallback; transaction pooler (6543) for serverless only. SQLAlchemy: `pool_pre_ping=True`, explicit pool options set (asyncpg defaults to NullPool otherwise). Celery: NullPool per worker task (fresh connection; no cross-fork pool sharing — supersedes any `engine.dispose(close=False)` guidance). Alembic as `postgres` role; `ALTER SYSTEM` not in Supabase's config surface (per docs; exact allowlist behavior unverified empirically). pgvector = `vector` extension; uuid-ossp enabled by default (per docs, not re-verified on this project). Realtime: tables must be added to `supabase_realtime` publication; logical replication already on. Open: Render IPv6 egress to direct endpoint (determines direct vs session pooler), psycopg2/pooler prepared-statement behavior, SQLite→Postgres copy translation.
