...
4. **Data**: locally observed state at migration time - `test.db` empty, local docker Postgres offline, no `PROD_DATABASE_URL` in `.env`. **Production data presence on Render was NOT queried** - if the Render prod DB holds data, dump it before cutover.
