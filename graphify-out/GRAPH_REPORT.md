...
## Corpus Check
- 68 files · ~16,675 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 579 nodes · 717 edges · 50 communities (35 shown, 15 thin omitted)
- Extraction: 90% EXTRACTED · 10% INFERRED · 0% AMBIGUOUS · INFERRED: 74 edges (avg confidence: 0.74)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `f277f82b`
...
Cohesion: 0.05
Nodes (54): Base, BaseModel, get_system_overview(), create_tag(), create_subtask(), create_task(), DeclarativeBase, AuditLog (+46 more)

### Community 1 - "User Authentication"
Cohesion: 0.06
Nodes (52): access_token_expire_time(), authenticate_user(), confirm_token_expire_time(), create_access_token(), create_confirm_token(), create_credentials_exception(), create_reset_token(), get_current_user() (+44 more)

### Community 2 - "SQLAlchemy Models"
Cohesion: 0.06
Nodes (32): get_query_token(), API-specific dependencies.  These dependencies are specific to the API routers, Optional query token validation for API routes.      This is a simple query-ba, Optional query token validation for API routes.          This is a simple quer, get_db(), get_query_token(), get_token_header(), Shared dependencies used across multiple routes and modules.  This module contai (+24 more)
...
Cohesion: 0.5
Nodes (3): Test that the seeding script successfully populates a confirmed user,     with r, Test that the seeding script successfully populates a confirmed user,     with r, test_seed_data()

## Knowledge Gaps
- **191 isolated node(s):** `Project structure overview and getting started guide.  This file provides a qu`, `Run migrations in 'offline' mode.      This configures the context with just a U`, `Run migrations in 'online' mode.      In this scenario we need to create an Engi`, `Initial migration  Revision ID: a6e267909ed1 Revises: Create Date: 2026-05-0`, `Add audit logs table  Revision ID: e7d04c90bc13 Revises: a6e267909ed1 Create Dat` (+186 more)
...
- **Why does `FastAPI Framework` connect `SQLAlchemy Models` to `Background Tasks & Email`, `User Authentication`, `Audit Logging System`, `Pytest Fixtures`?**
  _High betweenness centrality (0.182) - this node is a cross-community bridge._
- **Why does `SQLAlchemy ORM` connect `Database Migrations` to `SQLAlchemy Models`, `Audit Logging System`, `App Configuration`, `Statistics Tests`, `Internal Modules`, `Model Layer`, `Tests Suite`, `Misc Utils`?**
  _High betweenness centrality (0.178) - this node is a cross-community bridge._
- **Why does `User` connect `Database Migrations` to `Security Tests`?**
