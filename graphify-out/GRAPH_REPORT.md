...
## Corpus Check
- 74 files · ~21,054 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 773 nodes · 1012 edges · 78 communities (52 shown, 26 thin omitted)
- Extraction: 87% EXTRACTED · 13% INFERRED · 0% AMBIGUOUS · INFERRED: 130 edges (avg confidence: 0.7)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `800f9625`
...
Cohesion: 0.21
Nodes (19): access_token_expire_time(), authenticate_user(), confirm_token_expire_time(), create_access_token(), create_confirm_token(), create_credentials_exception(), get_current_user(), get_password_hash() (+11 more)

### Community 10 - "FastAPI Dependencies"
Cohesion: 0.11
Nodes (19): get_my_profile(), Retrieve the current user's profile information., Update the current user's username.     Checks for uniqueness and length., Retrieve the current user's profile information., Retrieve the current user's profile information., Retrieve the current user's profile information., Retrieve the current user's profile information., Update the current user's username.     Checks for uniqueness and length. (+11 more)
...
Cohesion: 0.11
Nodes (6): mock_httpx_client(), Mock httpx.AsyncClient to prevent real HTTP requests during tests., Mock httpx.AsyncClient to prevent real HTTP requests during tests., Mock httpx.AsyncClient to prevent real HTTP requests during tests., Mock httpx.AsyncClient to prevent real HTTP requests during tests., Mock httpx.AsyncClient to prevent real HTTP requests during tests.

### Community 14 - "Migration Tests"
Cohesion: 0.12
Nodes (17): Placeholder for the reset password page.     In a real app, this would be handl, Placeholder for the reset password page.     In a real app, this would be handl, Update the current user's password securely.     Verifies the current password, Initiate password reset flow by sending an email with a reset token., Reset user password using a valid reset token., Reset user password using a valid reset token., Reset user password using a valid reset token., Placeholder for the reset password page.     In a real app, this would be handl (+9 more)

### Community 15 - "Project Metadata"
Cohesion: 0.21
Nodes (11): get_async_database_url(), _handle_postgresql(), _handle_sqlite(), Ensures the database URL uses an async driver., Applies async driver and thread safety settings for SQLite., Extracts and sanitizes PostgreSQL connection arguments., Applies async driver and sanitizes query parameters for PostgreSQL., Ensures the database URL uses an async driver and handles driver-specific query (+3 more)
...
Cohesion: 0.18
Nodes (11): blacklist_token(), Blacklist a JWT token in Redis with a TTL., Blacklist a JWT token in Redis with a TTL., logout(), Logout the current user.     Since the application uses stateless JWTs, the cli, Logout the current user.     Since the application uses stateless JWTs, the cli, Logout the current user.     Since the application uses stateless JWTs, the cli, Logout the current user.     Since the application uses stateless JWTs, the cli (+3 more)
...
Cohesion: 0.24
Nodes (8): SystemOverview, TagDistribution, TaskStats, get_system_overview(), Retrieve a summary of tasks and tag distribution for the current user., Retrieve a summary of tasks and tag distribution for the current user., Retrieve a summary of tasks and tag distribution for the current user., Retrieve a summary of tasks and tag distribution for the current user.

### Community 25 - "Security Validation"
Cohesion: 0.2
Nodes (9): FastAPI Framework, confirm_email(), Resend a confirmation email for an existing, unconfirmed user., Resend a confirmation email for an existing, unconfirmed user., Resend a confirmation email for an existing, unconfirmed user.      The endpoi, Resend a confirmation email for an existing, unconfirmed user.      The endpoi, Resend a confirmation email for an existing, unconfirmed user., Resend a confirmation email for an existing, unconfirmed user. (+1 more)
...
Cohesion: 0.22
Nodes (9): Update the current user's password securely.     Verifies the current password, Update the current user's password securely.     Verifies the current password, Update the current user's password securely.     Verifies the current password, Update the current user's password securely.     Verifies the current password, Update the current user's password securely.     Verifies the current password, Update the current user's password securely.     Verifies the current password, Update the current user's password securely.     Verifies the current password, Update the current user's password securely.     Verifies the current password (+1 more)
...
Cohesion: 0.25
Nodes (8): forgot_password(), Initiate password reset flow by sending an email with a reset token., Initiate password reset flow by sending an email with a reset token., Initiate password reset flow by sending an email with a reset token., Logout the current user., Initiate password reset flow by sending an email with a reset token., Initiate password reset flow by sending an email with a reset token., Initiate password reset flow by sending an email with a reset token.

### Community 35 - "Community 35"
Cohesion: 0.33
Nodes (5): get_audit_logs(), Retrieve audit logs for the current user., Retrieve audit logs for the current user., Retrieve audit logs for the current user., Retrieve audit logs for the current user.
...
## Knowledge Gaps
- **262 isolated node(s):** `Project structure overview and getting started guide.  This file provides a qu`, `Run migrations in 'offline' mode.      This configures the context with just a U`, `Run migrations in 'online' mode.      In this scenario we need to create an Engi`, `Initial migration with Projects  Revision ID: 67fff7e05b96 Revises:  Create Date`, `Fail-fast in production when SECRET_KEY is not set.` (+257 more)
...
- **Why does `SQLAlchemy ORM` connect `Community 41` to `Database Migrations`, `User Authentication`, `Task Creation Logic`, `Statistics Tests`, `Project Metadata`, `API Package`, `CRUD Layer`, `Schema Layer`, `Misc Utils`, `Misc Init`, `Community 33`, `Community 36`, `Community 38`, `Community 39`, `Community 40`, `Community 50`, `Community 51`, `Community 52`, `Community 53`, `Community 54`, `Community 55`?**
  _High betweenness centrality (0.216) - this node is a cross-community bridge._
- **Why does `FastAPI Framework` connect `Security Validation` to `User Authentication`, `Community 35`, `App Configuration`, `Pytest Fixtures`, `Community 42`, `Seed Data Scripts`, `Community 45`, `Community 51`, `Tests Suite`, `Misc Utils`, `Community 31`?**
  _High betweenness centrality (0.149) - this node is a cross-community bridge._
- **Why does `TaskPriority` connect `Database Migrations` to `Community 43`?**
