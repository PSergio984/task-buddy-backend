# Graph Report - task-buddy-backend  (2026-05-10)

## Corpus Check
- 76 files · ~20,200 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 671 nodes · 838 edges · 71 communities (53 shown, 18 thin omitted)
- Extraction: 90% EXTRACTED · 10% INFERRED · 0% AMBIGUOUS · INFERRED: 83 edges (avg confidence: 0.72)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `6bb0633e`
...
2. `log_action()` - 17 edges
3. `Task Buddy Backend` - 15 edges
4. `send_confirmation_email()` - 13 edges
5. `get_task()` - 13 edges
6. `FastAPI Framework` - 13 edges
7. `Frontend Lint Cleanup Implementation Plan` - 11 edges
8. `Tag` - 10 edges
9. `Task` - 10 edges
10. `User` - 10 edges
...
Cohesion: 0.1
Nodes (31): create_audit_log(), log_action(), Log an action performed by a user on a specific target resource., Log an action performed by a user on a specific target resource., Log an action performed by a user on a specific target resource., create_group(), delete_group(), get_group() (+23 more)
...
Cohesion: 0.15
Nodes (14): Base, create_tag(), create_subtask(), create_task(), DeclarativeBase, AuditLog, Base, Group (+6 more)
...
Cohesion: 0.18
Nodes (15): BaseModel, AuditLog, AuditLogCreate, SubTaskCreateRequest, SubTaskCreateResponse, SubTaskUpdateRequest, TaskCreateRequest, TaskCreateResponse (+7 more)

### Community 13 - "Statistics Tests"
Cohesion: 0.28
Nodes (14): access_token_expire_time(), authenticate_user(), confirm_token_expire_time(), create_access_token(), create_confirm_token(), create_credentials_exception(), get_current_user(), _get_secret_key() (+6 more)

### Community 14 - "Migration Tests"
Cohesion: 0.15
Nodes (14): get_password_hash(), confirm_email(), Placeholder for the reset password page.     In a real app, this would be handl, Placeholder for the reset password page.     In a real app, this would be handl, Placeholder for the reset password page.     In a real app, this would be handl, Placeholder for the reset password page.     In a real app, this would be handl, Reset user password using a valid reset token., Reset user password using a valid reset token. (+6 more)

### Community 15 - "Project Metadata"
Cohesion: 0.15
Nodes (13): get_my_profile(), Retrieve the current user's profile information., Update the current user's username.     Checks for uniqueness and length., Retrieve the current user's profile information., Retrieve the current user's profile information., Retrieve the current user's profile information., Retrieve the current user's profile information., Update the current user's username.     Checks for uniqueness and length. (+5 more)
...
Cohesion: 0.17
Nodes (12): forgot_password(), logout(), Logout the current user.     Since the application uses stateless JWTs, the cli, Logout the current user.     Since the application uses stateless JWTs, the cli, Logout the current user.     Since the application uses stateless JWTs, the cli, Logout the current user.     Since the application uses stateless JWTs, the cli, Logout the current user.     Since the application uses stateless JWTs, the cli, Initiate password reset flow by sending an email with a reset token. (+4 more)
...
Cohesion: 0.22
Nodes (8): Run migrations in 'offline' mode.      This configures the context with just a U, Run migrations in 'offline' mode.      This configures the context with just a U, Run migrations in 'offline' mode.      This configures the context with just a, Run migrations in 'online' mode.      In this scenario we need to create an Engi, Run migrations in 'online' mode.      In this scenario we need to create an Engi, Run migrations in 'online' mode.      In this scenario we need to create an En, run_migrations_offline(), run_migrations_online()
...
Cohesion: 0.28
Nodes (7): SystemOverview, TagDistribution, TaskStats, get_system_overview(), Retrieve a summary of tasks and tag distribution for the current user., Retrieve a summary of tasks and tag distribution for the current user., Retrieve a summary of tasks and tag distribution for the current user.
...
Cohesion: 0.25
Nodes (7): ForgotPasswordRequest, Login, PasswordUpdate, ResetPasswordRequest, User, UserIn, UsernameUpdate
...
Cohesion: 0.32
Nodes (7): SubTaskCreateRequest, SubTaskCreateResponse, SubTaskUpdateRequest, TaskCreateRequest, TaskCreateResponse, TaskUpdateRequest, TaskWithSubTasks
...
Cohesion: 0.29
Nodes (7): Update the current user's password securely.     Verifies the current password, Update the current user's password securely.     Verifies the current password, Update the current user's password securely.     Verifies the current password, Update the current user's password securely.     Verifies the current password, Update the current user's password securely.     Verifies the current password, Update the current user's password securely.     Verifies the current password, update_password()

### Community 35 - "Community 35"
Cohesion: 0.33
Nodes (4): AuditLogCreate, TagCreate, TagResponse, SQLAlchemy ORM

### Community 36 - "Community 36"
Cohesion: 0.47
Nodes (5): downgrade(), get_fk_name(), add_group_model_v2  Revision ID: 38d622ba3db7 Revises: e7d04c90bc13 Create D, Helper to find foreign key name dynamically., upgrade()

### Community 37 - "Community 37"
Cohesion: 0.33
Nodes (5): FastAPI Framework, get_audit_logs(), Retrieve audit logs for the current user., Retrieve audit logs for the current user., Retrieve audit logs for the current user.
...
Cohesion: 0.7
Nodes (4): Enum, AuditAction, TaskStatus, str

### Community 41 - "Community 41"
Cohesion: 0.4
Nodes (5): Resend a confirmation email for an existing, unconfirmed user.      The endpoi, Resend a confirmation email for an existing, unconfirmed user.      The endpoi, Resend a confirmation email for an existing, unconfirmed user., Resend a confirmation email for an existing, unconfirmed user., resend_confirmation()
...
Cohesion: 0.6
Nodes (4): GroupBase, GroupCreateRequest, GroupResponse, GroupUpdateRequest

### Community 46 - "Community 46"
Cohesion: 0.5
Nodes (3): Admin module.  Contains admin-only functionality and operations. Not exposed, Admin update endpoint., update_admin()
...
Cohesion: 0.5
Nodes (3): Test that the seeding script successfully populates a confirmed user,     with r, Test that the seeding script successfully populates a confirmed user,     with, test_seed_data()
...
## Knowledge Gaps
- **224 isolated node(s):** `Project structure overview and getting started guide.  This file provides a qu`, `Run migrations in 'offline' mode.      This configures the context with just a`, `Run migrations in 'online' mode.      In this scenario we need to create an En`, `add_group_model_v2  Revision ID: 38d622ba3db7 Revises: e7d04c90bc13 Create D`, `Helper to find foreign key name dynamically.` (+219 more)
...
- **Why does `SQLAlchemy ORM` connect `Community 35` to `Community 32`, `User Authentication`, `SQLAlchemy Models`, `Community 33`, `Community 36`, `App Configuration`, `Community 38`, `Seed Data Scripts`, `API Routers`, `Community 51`, `Model Layer`, `Misc Init`, `Community 28`, `Community 29`, `Community 30`?**
  _High betweenness centrality (0.191) - this node is a cross-community bridge._
- **Why does `FastAPI Framework` connect `Community 37` to `User Authentication`, `Security Tests`, `Pytest Fixtures`, `Community 42`, `Statistics Tests`, `Migration Tests`, `Community 46`, `Community 51`, `Schema Layer`, `Docker Configuration`, `Community 28`?**
  _High betweenness centrality (0.173) - this node is a cross-community bridge._
- **Why does `User` connect `App Configuration` to `Audit Log Tests`, `Audit Logging System`?**
...
- **What connects `Project structure overview and getting started guide.  This file provides a qu`, `Run migrations in 'offline' mode.      This configures the context with just a`, `Run migrations in 'online' mode.      In this scenario we need to create an En` to the rest of the system?**
