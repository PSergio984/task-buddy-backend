...
async def test_audit_task_creation(async_client: AsyncClient, logged_in_token: str) -> None:
    ...
    response = await async_client.post(
        "/api/v1/tasks/", json=task_body, headers={"Authorization": f"Bearer {logged_in_token}"}
    )
    ...
    relevant_log = next(
        (
            log
            for log in logs
            if log["target_type"] == "TASK" and log["action"] == AuditAction.CREATE
        ),
        None,
    )
...
async def test_audit_project_creation(async_client: AsyncClient, logged_in_token: str) -> None:
    ...
    relevant_log = next(
        (
            log
            for log in logs
            if log["target_type"] == "PROJECT" and log["action"] == AuditAction.CREATE
        ),
        None,
    )
    assert relevant_log is not None
...
async def test_audit_tag_creation(async_client: AsyncClient, logged_in_token: str) -> None:
    ...
    relevant_log = next(
        (
            log
            for log in logs
            if log["target_type"] == "TAG" and log["action"] == AuditAction.CREATE
        ),
        None,
    )
    assert relevant_log is not None
