...
async def test_reset_password_success(
    ...
    response = await async_client.post(
        "/api/v1/users/reset-password", json={"token": reset_token, "new_password": new_password}
    )
    ...
    login_response = await async_client.post(
        "/api/v1/users/token", data={"username": confirmed_user["email"], "password": new_password}
    )
...
async def test_reset_password_too_short(
    ...
    response = await async_client.post(
        "/api/v1/users/reset-password", json={"token": reset_token, "new_password": "short"}
    )
